...
token = 'token_123@_inventory_to_products'
...
async def read_product(product_id: int, is_authorized: bool = Depends(verify_internal_token) ,db: Session = Depends(get_db)):
    ...
    try:
        #Definimos url+servicio+product_id, nombre_servicio, token para consultar INVENTORY
        #la funcion construye el header
        INVENTORY_SERVICE_URL = f'http://127.0.0.1:8001/inventory/{product_id}'
        stock_actual = await call_external_service(INVENTORY_SERVICE_URL, 'INVENTORY-SERVICE', token)
    ...
    product_data['stock'] = stock_actual.get('cantidad')
